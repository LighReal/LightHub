...
local HubName = "Light Hub|Blox Fruits by LightReall"
...
local function RemoveKey()
    local s, e = pcall(function()
        if isfile('LightReal.bin') then
            delfile('LightReal.bin')
        end
    end)
end
...
local function wl(key)
    ...
    if sign_match == decode_response.sign then
        ...
        Hub = {}
        Hub.Name = "Light Hub"
        Hub.Game = "Blox Fruits By LightReall"
        ---- Settings -----
        ...
        function Window:NewTab(mmb)
            ...
            function newmm:NewParagraph(cf)
                local paragraphnew
                newTitle = cf.Title or "Light Hub"
                ...
                function paragraphnew:Set(cf2)
                    newTitle = cf2.Title or "Light Hub"
        ...
        Fluent:Notify(
            {
                Title = "Light Hub Hub",
                Content = " loaded Xong Ban Oi successful.",
                Duration = 15
            }
        )
            else
